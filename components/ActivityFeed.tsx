...
function EventRow({ event }: { event: AgentEvent }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-gray-700/50 animate-fade-in">
      <span className="text-base mt-0.5 shrink-0">{emoji}</span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-xs font-semibold text-gray-300 capitalize">{event.agent}</span>
          <span
            className={clsx(
              'text-xs px-1.5 py-0.5 rounded font-medium',
              event.status === 'ok' && 'bg-emerald-500/10 text-emerald-400',
          ...
          <span className="text-xs text-gray-600 ml-auto shrink-0">{timeAgo}</span>
        </div>
        <p className="text-xs text-gray-400 leading-snug">{event.event}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className={clsx('bg-gray-800 rounded-xl border border-gray-700', className)}>
      ...
      <div className="px-4 overflow-y-auto max-h-96">
        {events.length === 0 ? (
          <p className="text-sm text-gray-500 py-8 text-center">No activity yet. Agents will log here.</p>
        ) : (
